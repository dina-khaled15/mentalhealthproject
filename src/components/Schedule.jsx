...
import {Box,Button,Typography,Grid,Card,CardContent,LinearProgress,Chip} from "@mui/material";
...
const styles = {
  ...
  Wednesday: (slot) => {
    ...
    } else if (slot === "8:00 AM" || slot === "2:00 PM" || slot === "3:00 PM") {
      return {
        backgroundColor: "#EEEEEE", 
        color: "#B5B5B5",
        border: "1px solid #B5B5B5",
...
export default function BookingSchedule() {
  ...
  return (
    <Box sx={{ height: "100vh", display: "flex", overflow: "hidden" }}>
      <Box
        sx={{width: "35%",p: 4,display: "flex",flexDirection: "column",bgcolor: "#fff",
        }}
      >
        <Button
          variant="outlined"
          sx={{width: "150px",height: "35px",borderRadius: "20px",border: "1px solid #D3D3D3",backgroundColor: "#fff",
            color: "#222222",mb: 1.5,textTransform: "none",}}>
          <RocketLaunchIcon sx={{ fontSize: "20px" }} />
          Book Session
        </Button>
        <Typography variant="h5" fontWeight="400" fontSize={"45px"}>
          Reserve Your Spot!<br />We’ve Got a Schedule <br />for You
        </Typography>
        <Typography variant="body2" sx={{ mt: 2, color: "gray", fontFamily: "Manrope", fontSize: "20px", fontWeight: 400 }}>
          Our upcoming counseling sessions, designed to<br /> support your mental health journey
        </Typography>
      </Box>

      <Box
        sx={{width: "65%",p: 2,overflowY: "auto",display: "flex", flexDirection: "column",gap: 2,bgcolor: "#fff",}}>
        {scheduleData.map((day, index) => (
          <Card
            key={index}
            variant="outlined"
            sx={{width: "55%",alignSelf: "center",height: "280px",display: "flex",flexDirection: "column",
              backgroundColor: "#F8F7F4",borderRadius: "10px",}}>
            <CardContent sx={{ overflow: "hidden", p: 1 }}>
              <Grid container justifyContent="space-between" alignItems="center">
                <Grid item>
                  <Typography variant="subtitle2" sx={{ color: "black", fontFamily: "Monarp, sans-serif" }}>
                    {day.day} - {day.date}
                  </Typography>
                </Grid>
                <Grid item>
                  <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                    <LinearProgress
                      variant="determinate"
                      value={day.booked}
                      sx={{height: 5,borderRadius: 5,width: 80,backgroundColor: "#CAC8C2",
              ...
              <Box sx={{ mt: 0.5, display: "flex", justifyContent: "flex-end" }}>
                <Typography
                  ...
                  <span
                    style={{fontWeight: 750,fontSize: "15px",fontFamily: "Manrope",lineHeight: "17px",color: "#000000",marginRight: "6px"}}>
                    {day.available}
                  </span>
                ...
                {chunkSlots(day.slots, 6).map((slotChunk, i) => (
                  <Grid container spacing={2} key={i} sx={{ mb: 1 }}>
                    {slotChunk.map((slot, i) => {
                      let chipStyles = {borderRadius: "2px",fontSize: "10px",paddingRight: "3px",fontWeight: 600,
                        fontFamily: "Manrope",width: "65px",};
              ...
              <Box sx={{ mb: 7, display: "flex", justifyContent: "flex-end" }}>
                <Button
                  variant="contained"
                  size="small"
                  sx={{width: "90px",height: "28px",fontFamily: "Manrope",fontWeight: 600,fontSize: "10px",
