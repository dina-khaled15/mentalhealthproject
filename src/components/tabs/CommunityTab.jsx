...
const CommunityTab = () => {
    ...
    const fetchMessages = async () => {
      try {
        ...
        setMessages(data.map(msg => ({
          text: msg.text,
          received: msg.user.toString() !== localStorage.getItem('userId'),
          type: msg.type,
  ...
  const sendMessage = () => {
    if (message.trim()) {
      ...
      setMessages((prevMessages) => [...prevMessages, { text: message, received: false, type: 'text' }]);
      setMessage('');
    }
  };
    ...
    if (mediaRecorder && isRecording) {
      ...
      setTimeout(async () => {
        if (audioChunks.length > 0) {
          ...
          try {
            const token = localStorage.getItem('token');
            const response = await fetch('http://localhost:4000/api/messages/audio', {
              method: 'POST',
              headers: {
                'Authorization': `Bearer ${token}`,
              },
